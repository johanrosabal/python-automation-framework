...
logger = setup_logger('OriginInformationPage')


class OriginInformationPage(BasePage):
    ...
    def __init__(self, driver):
        ...
        # Locator definitions
        self._form = "(//div[contains(@class,'origin-info')])[2]"
        ...
        self._input_early_pickup_date = (By.XPATH, "//input[@name='earlyPickUpDate']", "From Day [Pickup Days Input Box]")  # Only Visible when First Come is Selected
        self._input_late_pickup_date = (By.XPATH, "//input[@name='latePickUpDate']", "To Day [Pickup Days Input Box]")  # These are visible on the 3 Radio Options
        ...
        self._input_first_and_last_name = (By.NAME, "pickupContactName","Contact name [Input Box]")
        self._input_phone = (By.NAME,  "pickupContactPhone", "Phone [Input Box]")
        self._input_email = (By.NAME, "pickupContactEmail", "Email [Input Box]")
    ...
    def get_instance(cls):
        if not hasattr(cls, '_instance'):
            cls._instance = OriginInformationPage(BaseApp.get_driver())
    ...
    def _select_autocomplete_location_name_by_location(self, text: str):
        locator = (By.XPATH, f"({self._form}//*[contains(@class,'dropdown-item')]/div/div[1]/span[contains(text(), '{text}')])[1]", "Location Option Location Name [Dropdown Locations Autocomplete]")
        self.click().set_locator(locator, self._name).single_click()
        return self

    def _select_autocomplete_location_name_by_address(self, text: str):
        locator = (By.XPATH, f"({self._form}//*[contains(@class,'dropdown-item')]/div/div[2]/span[contains(text(), '{text}')])[1]", "Location Option Location Address [Dropdown Locations Autocomplete]")
        self.click().set_locator(locator, self._name).single_click()
        return self

    def _select_autocomplete_location_name_by_contact_email(self, text: str):
        locator = (By.XPATH, f"({self._form}//*[contains(@class,'dropdown-item')]/div/div[2]/span[2][contains(text(), '{text}')]", "Location Option Location Contact Email [Dropdown Locations Autocomplete]")
        self.click().set_locator(locator, self._name).single_click()
        return self
    ...
    def enter_origin_location(self, name, location, address, address_line_2):
        # Location Name
    ...
    def _enter_open_time(self, hours: str, minutes: str):
        ...
        self.send_keys().set_locator(self._input_open_time_minutes, self._name).clear().set_text(str(minutes)).pause(1).set_text(Keys.TAB)
    ...
    def _enter_time(self, hours: str, minutes: str):
        ...
        self.send_keys().set_locator(self._input_time_minutes, self._name).clear().set_text(str(minutes))
